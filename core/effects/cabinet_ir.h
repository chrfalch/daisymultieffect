...
struct CabinetIREffect : BaseEffect
{
    ...
    void SetParam(uint8_t id, float v) override
    {
        ...
        {
            case 0: mix_ = v; break;
            case 1: outputGain_ = v; break;
            case 2: lowCut_ = v; break;
            case 3: highCut_ = v; break;
        }
    }
    ...
    void ProcessStereo(float& l, float& r) override
    {
        ...
        // lowCut_ 0-1 maps to 20Hz-500Hz
        float hpFreq = 20.0f + lowCut_ * 480.0f;
        float hpCoeff = 1.0f - std::exp(-2.0f * 3.14159f * hpFreq / sampleRate_);
        hpfState_ += hpCoeff * (wet - hpfState_);
        wet = wet - hpfState_;
        
        // Apply low-pass filter (high cut)
        // highCut_ 0-1 maps to 2kHz-20kHz
        float lpFreq = 2000.0f + highCut_ * 18000.0f;
