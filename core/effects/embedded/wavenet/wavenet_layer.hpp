/**
 * @file wavenet_layer.hpp
 * @brief WaveNet layer implementation for NAM models
 *
 * From GuitarML/Mercury's wavenet implementation.
 * Original: https://github.com/GuitarML/Mercury/blob/main/wavenet/wavenet_layer.hpp
 * License: MIT
 */
#pragma once
...
namespace wavenet
{
    ...
    struct Wavenet_Layer
    {
        ...
        void load_weights(Iterator &weights)
        {
            conv.reset();

            std::vector<std::vector<std::vector<float>>> conv_weights(
                channels, std::vector<std::vector<float>>(channels, std::vector<float>(kernel_size)));
