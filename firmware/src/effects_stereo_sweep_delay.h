...
struct StereoSweepDelayEffect : TimeSyncedEffect
{
    ...
    static float bufL[MAX_SAMPLES];
    static float bufR[MAX_SAMPLES];
    int wp = 0;
    ...
    explicit StereoSweepDelayEffect(TempoSource &t) : TimeSyncedEffect(t) {}
    ...
    void ProcessStereo(float &l, float &r) override
    {
        ...
        bufL[wp] = inMono + dl * feedback_;
        bufR[wp] = inMono + dr * feedback_;
        if (++wp >= MAX_SAMPLES)
            wp = 0;
