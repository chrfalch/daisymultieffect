
#pragma once
#include <cstdint>

struct ParamDesc { uint8_t id; uint8_t value; }; // 0..127 snapshot

enum class ChannelMode : uint8_t { Mono, Stereo, MonoOrStereo };

enum class ParamValueKind : uint8_t { Number = 0, Enum = 1 };

struct NumberParamRange { float minValue; float maxValue; float step; };
struct EnumParamOption { uint8_t value; const char* name; };
struct EnumParamInfo { const EnumParamOption* options; uint8_t numOptions; };

struct ParamInfo
{
    uint8_t id;
    const char* name;
    const char* description;
    ParamValueKind kind;
    const NumberParamRange* number;      // if Number
    const EnumParamInfo* enumeration;    // if Enum
};

struct EffectMeta
{
    const char* name;
    const char* description;
    const ParamInfo* params;
    uint8_t numParams;
};

struct BaseEffect
{
    virtual ~BaseEffect() {}
    virtual uint8_t GetTypeId() const = 0;
    virtual ChannelMode GetSupportedModes() const = 0;

    virtual void Init(float sampleRate) = 0;
    virtual void ProcessStereo(float& l, float& r) = 0;

    virtual void ProcessMono(float& m)
    {
        float l=m, r=m;
        ProcessStereo(l,r);
        m = 0.5f*(l+r);
    }

    virtual void SetParam(uint8_t id, float v01) = 0; // v in [0,1]
    virtual uint8_t GetParamsSnapshot(ParamDesc* out, uint8_t max) const = 0;

    virtual const EffectMeta& GetMetadata() const = 0;
};
